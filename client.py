...
def openChatWindow():
    ...
    textarea = Text(window,width=60,height=5,font = ("Calibri",10))
    ...
    text_message = Entry(window,width=30,font = ("Calibri",10))
    text_message.pack()
    text_message.place(x=140,y=300)
    
    attach = Button(window,text="Attech and Send",bd=1, font = ("Calibri",10))
    attach.place(x=7,y=300)

    filePathLabel = Label(window,text='')
    filePathLabel.place(x=10,y=325)

    send = Button(window,text="Send",bd=1, font = ("Calibri",10))
    send.place(x=400,y=300)
